...
from kate.core.websocket.escape import to_unicode, json_encode, utf8, native_str
from kate.core.websocket.util import _websocket_mask_python as _websocket_mask
...
class WebSocketHandler:
    ...
    async def get(self, *args: 'Any', **kwargs: 'Any'):
        ...
        # might mess with it.
        connection = map(
            lambda s: s.strip().lower(), self.headers.get("Connection", "").split(",")
        )
    ...
    def set_nodelay(self, value: bool) -> None:
        ...
        """
        assert self.ws_connection is not None
        self.ws_connection.set_nodelay(value)  # TODO: to socket of the server
    ...
    def set_status(self, status_code: int, reason: Optional[str] = None) -> None:  # to remove
    ...
    def set_header(self, name: str, value) -> None:  # to remove
    ...
    async def finish(self, chunk: Optional[Union[str, bytes, dict]] = None) -> "Future[None]":  # to remove
...
class WebSocketProtocol(abc.ABC):
    ...
    def __init__(self, handler: "_WebSocketDelegate", reader, writer) -> None:
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    def _handle_websocket_headers(self, handler: WebSocketHandler) -> None:
        ...
        if not all(map(lambda f: handler.headers.get(f), fields)):
    ...
    async def close(self, code: Optional[int] = None, reason: Optional[str] = None) -> None:
        ...
        if not self.server_terminated:
            ...
            close_frame = await self._write_frame(b"", 0x8)
